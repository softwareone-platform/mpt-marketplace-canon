...
| BR-002 | A Product may be deleted by its Vendor only while in Draft state (T7). Deletion is a soft delete and cascades: [[Item Group]]s, [[Parameter]]s, [[Parameter Group]]s, [[Template]]s, [[Terms]] (and [[Terms Variant]]s), [[Price List]]s (and [[Price List Item]]s), [[Authorization]]s, and [[Listing]]s are removed, along with the Product's Documents, [[Media]], and Icon. Product [[Item]]s are independent records and are not removed by this deletion. Once a Product leaves Draft state (Pending, Published, or Unpublished), it can never be deleted by any Actor. | Draft (for the transition itself); all other states are permanently non-deletable | Vendor | This is a documented exception to Platform Invariant 6 — see preamble. |
...
| Catalog: Product Item | Child | 1:Many | A Product owns a collection of Items (orderable SKUs). | No — Items are independent records and are not removed when the Product is deleted (unlike the other child types above). |
...
| Product deleted | T7 — Draft to Deleted | Vendor | Cascade-deletes [[Item Group]]s, [[Parameter]]s, [[Parameter Group]]s, [[Template]]s, [[Terms]] (and [[Terms Variant]]s), [[Price List]]s (and [[Price List Item]]s), [[Authorization]]s, and [[Listing]]s; removes Documents/Media/Icon. Product [[Item]]s are independent and are not removed. Permanently removed — no longer retrievable via the API. |
...
| Product deleted (Draft only) | Product Item Group, Product Parameter, Product Parameter Group, Product Template, Product Terms (and Variants), Price List (and Price List Item), Authorization, Listing, Media, Documents, Icon | Removed alongside the Product. | Yes | Product must be in Draft state (BR-002) | Product Items are not removed — they are independent records. Since only Draft Products qualify, none of the removed objects should have live [[Order]]s/[[Agreement]]s/[[Subscription]]s attached yet. |
...
**Deletion:**
A Product may be deleted by its Vendor only while in Draft state (T7). This is a soft delete — the object is excluded from all normal API responses going forward (permanently removed — no longer retrievable via the API) — and it cascades to remove: [[Item Group]]s, [[Parameter]]s, [[Parameter Group]]s, [[Template]]s, [[Terms]] (and [[Terms Variant]]s), [[Price List]]s (and [[Price List Item]]s), [[Authorization]]s, and [[Listing]]s, plus Documents, Media, and Icon. Product [[Item]]s are independent records and are not removed by the Product's deletion. This is a documented exception to Platform Invariant 6 (see preamble). Once a Product leaves Draft state (Pending, Published, or Unpublished), it can never be deleted by any Actor.
...
| Vendor deletes a Draft Product that has child objects (Templates, Item Groups, etc.) already configured | The child objects listed in Section 8, plus Authorizations and Listings, are cascade-deleted with no confirmation step beyond the delete call itself. The Product's [[Item]]s are not removed — they survive as independent records. | Vendor | Medium | Since deletion is Draft-only, no live [[Order]]s/[[Agreement]]s/[[Subscription]]s can be affected — but configuration work on the cascade-deleted child objects is lost irrecoverably. |
