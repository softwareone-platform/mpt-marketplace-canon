...
| BR-003 | A Module's `accountTypes` declares which account types the Module applies to. A Module may only be referenced by a [[User Group]] or API Token permission set within an Account whose type is listed in the Module's `accountTypes`. | N/A | Operations | Observed values: `Vendor`, `Client`, `Operations`. User Group and API Token are not yet canonised. |
...
| Accounts: API Token | Association | Module (many) ↔ API Token (many) | An API Token's permission set references Modules in the same way as a User Group. API Token is not yet canonised. | No cascade. A referenced Module is not affected by API Token deletion. |
