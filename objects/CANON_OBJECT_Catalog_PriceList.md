...
| --- | --- | --- | --- | --- | --- |
| currency | String | ISO 4217 currency code (e.g. USD, EUR, JPY) | Vendor | No | Visible To: All. Immutable after creation. Required on creation. |
| precision | Integer | Number of decimal places for prices in this currency | Vendor | No | Visible To: All. Explicitly set by Vendor on creation. Immutable after creation. Required on creation. e.g. 2 for USD/EUR/GBP, 0 for JPY. |
| notes | String | Free-text field for Vendor-authored internal documentation about this Price List | Vendor | Yes | Visible To: Vendor, Operations. Optional. Absent from response when null. |
| defaultMarkup | Decimal | Fallback markup % applied during Billing: Charge reconciliation when a Charge cannot be matched to a Subscription or Agreement | Operations | Yes | Visible To: Operations only. Billing fallback only — not applied to Price List Items. |
| defaultMargin | Decimal | Computed gross margin % derived from defaultMarkup | System | N/A | Visible To: Operations only. Never stored. Computed on read. |
| externalIds.vendor | String | Vendor's own identifier for this Price List | Vendor | Yes | Visible To: Vendor, Operations. Optional. |
| statistics.sellers | Integer | Number of Sellers associated with this Price List via Listings | System | N/A | Visible To: All. Computed. Read-only. |
| statistics.listings | Integer | Number of Listings referencing this Price List | System | N/A | Visible To: All. Computed. Read-only. |
| statistics.priceListItems | Integer | Total number of Price List Items in this Price List | System | N/A | Visible To: All. Computed. Always equal to the total number of Items in the parent Product. |
| statistics.purchasePriceItems | Integer | Number of Price List Items with unitPP set | System | N/A | Visible To: All. Computed. Read-only. |
| statistics.purchasePriceCompleteness | Decimal | Percentage of Price List Items with unitPP set | System | N/A | Visible To: All. Computed. Read-only. |
| statistics.salesPriceItems | Integer | Number of Price List Items with unitSP set (i.e. with markup set) | System | N/A | Visible To: Operations only. Computed. Read-only. |
| statistics.salesPriceCompleteness | Decimal | Percentage of Price List Items with unitSP set | System | N/A | Visible To: Operations only. Computed. Read-only. |
| statistics.averageMarkup | Decimal | Average markup across all priced Price List Items | System | N/A | Visible To: Operations only. Computed. Read-only. |
| statistics.averageMargin | Decimal | Average margin across all priced Price List Items | System | N/A | Visible To: Operations only. Computed. Read-only. |
| revision | Integer | Increments on each update | System | N/A | Visible To: All |
| audit | Object | Records the `created` and `updated` events for this Price List, including timestamp and Actor reference for each | System | N/A | Visible To: All. Omitted from response by default — request via `select=+audit`. |
| Statistics | object | Computed platform metrics. Sub-fields: sellers, listings, priceListItems, purchasePriceItems, purchasePriceCompleteness (all visible to all actors); salesPriceItems, salesPriceCompleteness, averageMarkup, averageMargin (Operations only — reflect SoftwareOne margin). All computed, read-only. | system | — | — |
