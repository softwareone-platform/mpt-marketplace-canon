...
| `parameters.fulfillment` | Array | Asset-scoped fulfilment parameters written and maintained by the Vendor. Contains parameters with `phase: "Fulfillment"` and `scope: "Asset"`. | Vendor | Yes — Vendor only, subject to BR-019 | Parameters with `hidden=true` suppressed from Client API responses. Retained on Terminated Assets — not cleared on termination. |
...
| --- | --- | --- | --- |
| 0.5 | 2026-07-17 | Stu / canon-generate-batch | Terminology corrected while canonising Commerce: Order Asset: fulfilment actions are attributed to "the Vendor" (the Actor), not a "Vendor Extension" — Vendor fulfilment is manual-first and does not require an extension. §3.1 Draft state, BR-001, the `parameters.fulfillment` attribute row, and the §9 failure-mode rows updated. Also dropped the now-stale "not yet canonised" qualifier on the Commerce: Order Line references (§6/§7) now that Order Line and Order Asset are canonised. |
