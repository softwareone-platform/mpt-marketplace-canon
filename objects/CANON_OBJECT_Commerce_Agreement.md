...
| BR-018 | Split Billing is configured on the Agreement via the `/split` sub-resource and can be managed only by the Client or Operations — never the Vendor. Activation requires the [[Product]] to have Split Billing enabled, requires the [[Licensee]]'s default [[Buyer]] to be included, and cannot be repeated once active. | Active | Client, Operations | On activation, each [[Subscription]] is seeded with a split allocation to the default [[Buyer]] (100%; other buyers 0%). Allocation percentages are computed by the platform from each [[Buyer]]'s share of Subscription selling price, normalised to 100 — they are not free-form Actor input. A [[Buyer]] with existing Subscription-level allocations cannot be removed. |
...
| Commerce: Order Line | Association | One Agreement to many Lines (Entitlements) | All Lines across all Subscriptions and Assets, via the `/lines` endpoint. Called Entitlements in the UI. Commerce: Order Line is not yet canonised. | No direct lifecycle dependency. |
