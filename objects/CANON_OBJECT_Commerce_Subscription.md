...
| T5 | Active | Terminated | Vendor terminates directly | Vendor | Subscription must be Active | Via `/terminate` endpoint. `terminationDate` set automatically by platform. If this is the last Active Subscription on the Agreement, Agreement → Terminated. |
