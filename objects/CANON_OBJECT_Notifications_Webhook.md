...
| BR-003a | `objectType` is **derived** from `type` via a platform-hardcoded mapping — it cannot be set independently. Confirmed mappings: Order-family types (`ValidatePurchaseOrderDraft/Querying`, `ValidateChangeOrderDraft`, `ValidateConfigurationOrderDraft`, `ValidateTerminateOrder`, `SelectOrderLines`) → `Order`; Enrollment-family types (`ValidateEnrollmentDraft/Querying`, `ValidateReEnrollment`, `ValidateReEnrollmentQuerying`) → `Enrollment`; `ValidateAnswer` → `Answer`. These three are the only reachable `objectType` values. The public schema's `objectType` enum also lists `Account` and `Request`, but `Request` refers to an object that has been deprecated and removed from the platform, and `Account` was never a reachable value — the public schema still lists both, though neither is actually reachable. | All | All | — |
...
| BR-004a | Products can only be deleted while in Draft state (see `Catalog: Product` canon BR-002). Deleting a Product does not reactively touch any Webhook whose `criteria` anchors to it — the Webhook keeps its last-resolved `object` snapshot and its own `Enabled`/`Disabled` status; it is not disabled, flagged, or notified. A subsequent update to that Webhook (re-resolving the object reference) then fails, since deleted Products are excluded from all normal queries once deleted. Whether the same applies to Programs is unconfirmed — Programs are not yet canonised. | All | All | See `Catalog: Product` canon BR-002 for the deletion mechanics this row depends on. |
...
| BR-005a | Every `type` has a mandatory anchor criteria key that must be present or creation/update fails: Order-family types require `product.id`; Enrollment-family types require `program.id`; `ValidateAnswer` requires `form.id`. | All | All | — |
...
No open questions at this time. WBH-001, WBH-002, and WBH-004 were resolved and incorporated above (see BR-003a, BR-004a, BR-006a, and Section 9); WBH-003 was raised and explicitly descoped as an internal engineering detail rather than a canon-worthy business rule.
