...
| BR-004a | Deleting a Product does not reactively affect any Webhook whose `criteria` anchors to it — the Webhook is not disabled, flagged, or notified. | All | All | Products can only be deleted while in Draft state (`Catalog: Product` canon BR-002). The Webhook keeps its last-resolved `object` snapshot and its own status; a subsequent update to it then fails, since deleted Products are excluded from all normal queries. Whether the same applies to Programs is unconfirmed — Programs are not yet canonised. |
