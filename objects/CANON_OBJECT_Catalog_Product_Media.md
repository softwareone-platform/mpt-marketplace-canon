...
| Deleted | Permanently removed from Draft. No longer retrievable via the API. Reachable only from Draft (BR-005). | No | Yes |
...
| --- | --- | --- | --- | --- | --- |
| Type | Enum | One of: Image, Video | Vendor | No | Required on creation. Determines storage model. Immutable after creation. |
| Name | String | Internal name of the Media object | Vendor | Yes | Required on creation. |
| Description | String | Descriptive text for the Media object | Vendor | Yes | Required on creation. |
| Display Order | Integer | Controls presentation sequence on the Product page | Vendor | Yes | Required on creation. |
| File | Binary | The uploaded file. For Image type: the image itself. For Video type: a thumbnail image. | Vendor | Yes | Required on creation for both types. Uploaded as binary (multipart). |
| Status | Enum | One of: Draft, Pending, Published, Unpublished | System | Yes — via state transitions only | — |
| Filename | String | Original filename of the uploaded file | System | N/A | Set on upload. Applicable to Image type; for Video type reflects the thumbnail filename. |
| Size | Integer | File size in bytes | System | N/A | Set on upload. |
| Content Type | String | MIME type of the uploaded file (e.g. image/png) | System | N/A | Set on upload. |
| URL | String | External URL of the hosted video | Vendor | Yes | Required on creation for Video type. Not applicable to Image type. |
| Revision | Integer | Increments when the Media object's own attributes change | System | N/A | Read-only. |
