...
- **`/canon-submit-pr <namespace> <object>`** is the explicit human-triggered step that promotes a reviewed draft into `objects/` and opens a PR. It enforces **one commit per PR** (subsequent runs amend + `git push --force-with-lease`, never a second commit or a bare `--force`); a PR is **not** restricted to one platform object — related objects (e.g. a cross-object dependency discovered mid-generation) are expected to be bundled into the same PR when they belong together, subject to human confirmation of scope.
