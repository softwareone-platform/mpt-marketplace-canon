...
In the same round, ask for any **Confluence page(s)** documenting each object (business context — `canon-generate` Step 5.5; optional, may be none/one/several). Because the Step D workflow subagents run headless and may not have the interactively-authenticated Atlassian MCP, **the orchestrator fetches these pages itself** here (or in Step C), saves them to each object's `.evidence/<ns>_<obj>/<run>/confluence/`, and passes the saved paths into that object's Step D agent prompt — the agent reads the saved extract rather than fetching live.
...
1. Tell it to read `.claude/skills/canon-generate/SKILL.md`'s "Required reading" list in full, then perform the equivalent of that Skill's **Steps 1 (schema), 2 (live fetch), 3 (actor diff), 5.5 (business-context Confluence), and the research-and-draft-assembly parts of Steps 5 and 6** for its one object — passing in the object ID(s)/environment(s) gathered in Step B, and the path(s) to any Confluence extract the orchestrator saved for it (the agent reads those saved files for Step 5.5 rather than fetching live, since it may lack the Atlassian MCP).
...
6. Tell it to write its own draft to `.evidence/<namespace>_<object>/<run>/draft/...` (Section 10 left as a clearly-marked list of unresolved candidates, not finalized), and to return a small structured object via a JSON schema — never the full draft text, to keep the join payload light:
   ...
   ```
7. Tell it to emit **template-exact structure** so the draft parses against `.canon`'s templates without a repair pass (a Workflow subagent can't run `npm run validate`, so structure is on it to get right, matched literally against `templates/CANON_OBJECT_TEMPLATE.md` and the `.canon/templates/*.md` shapes):
   - **Section headings and table header/separator rows must match the template literally.** Every table separator is exactly `| --- |` per column (never padded like `|-----|`); do not rename headings (e.g. `### 3.1 States`, not `### 3.1 Lifecycle points`) or add/remove/reorder columns.
   - **§3 is one of two exact shapes, never a hybrid.** Either (a) the full state machine — `### 3.1 States` as the 4-column table `| State | Description | Initial State? | Terminal State? |`, `### 3.2 Transitions` as the 8-column table, `### 3.3 State Diagram`; or (b) if the object has no lifecycle of its own, the pure prose form ("This object has no state machine. …") with **no** `### 3.1/3.2/3.3` subsections at all. Never keep a "no state machine" sentence *and* a transitions table.
   - **§3.2 transition integrity:** every `From State`/`To State` is a single state name that exactly matches a row in this object's own §3.1 (no compound `A / B`, no parentheticals like `Active (or …)`, no `— (removed)`); a deletion/removal transition targets a real terminal state listed in §3.1 (e.g. a `Deleted` row), not a free-text placeholder. There is **at most one transition per ordered (From, To) state pair** — `.canon` keys transitions by that pair and rejects a second row for the same pair, so fold multiple mechanisms for the same pair into one row (Action/Endpoint/Actor describing both) rather than adding a duplicate row.
   - **§5 Key Attributes is exactly the 6 template columns** `| Attribute | Type | Description | Set By | Mutable After Creation? | Notes |` — do not add a 7th column (e.g. "Mutable After Draft?"); put any state-specific mutability nuance in the Notes cell.
   - **§1 `**Parent Object:**` is plain `Namespace: Object` (or `None — top-level object.`), NEVER a `[[WikiLink]]`.** A `[[...]]` in that field breaks `.canon`'s parent resolution and fails CI's corpus round-trip test (`every entity has a parent ref`) by orphaning the entity — and `npm run validate` does **not** catch it (only the corpus test does), so a Workflow subagent can't self-detect it. The graph edge to the parent still forms from the §1 Description prose, where the parent's name *is* wikilinked normally. Write e.g. `**Parent Object:** Accounts: Account`, never `**Parent Object:** Accounts: [[Account]]`.
   - **Do not copy the template's own instructional blockquotes into the draft.** `templates/CANON_OBJECT_TEMPLATE.md` contains `> **Guidance:** …` / `> **Guidance on business rules:** …` blockquotes that tell the author how to fill a section — these are scaffolding, not canon content, and must not appear in the emitted draft (strip them entirely, section by section).
