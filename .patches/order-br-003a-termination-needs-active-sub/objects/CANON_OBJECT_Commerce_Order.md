...
| Draft | The Order has been created and saved but not yet placed. The Client can edit ordering parameters and the Order can be updated by Operations. The Order has not yet been submitted for fulfilment. | No | No |

| Quoted | The Order has been marked as ready for Client placement. Functionally equivalent to Draft except that no parameter values can be edited. Signals to the Client that the Order is ready to be placed. Draft → Quoted is a one-way transition. | No | No |

| Processing | The Order has been placed by the Client and is being processed by the Vendor's fulfilment Extension. The Vendor can update ordering and fulfilment parameters and transition the Order to Querying, Completed, or Failed. | No | No |

| Querying | The Order has been paused by the Vendor pending Client action — either correction of invalid parameter values or completion of an external task. The Client can update ordering parameters. The Order must return to Processing before it can be Completed or Failed. | No | No |

| Completed | The Order has been successfully fulfilled. All Lines have been mapped to Subscriptions or Assets. Terminal state — no outbound transitions. | No | No |

| Failed | The Order could not be fulfilled. A `statusNotes` object may be present explaining the reason for failure. A new Order must be created to retry. Terminal state — no outbound transitions. | No | No |

| Deleted | The Order has been soft-deleted — it moves to Deleted status and remains retrievable via the API including in standard list responses. Only reachable from Draft or Quoted. Terminal state — no outbound transitions. Deviates from Platform Invariant 7 — see Section 8. | No | No |
...
| BR-003a | A Termination Order can only be created against an [[Agreement]] that has at least one non-Terminated [[Subscription]]. There must be at least one active commercial relationship to terminate. The platform rejects creation of a Termination Order against an [[Agreement]] whose Subscriptions are all Terminated. | — (creation) | Client | Applies only to Termination Orders. [[Asset]]s are not considered — they cannot be terminated and do not satisfy this precondition. Subscriptions in transient states such as `Updating` count as non-Terminated. |
...
| Commerce: Order | Child | One Order to many Lines | Lines are the unit of work within a Purchase, Change, or Termination Order — one Line per SKU at one quantity. Each Line must be mapped to a Subscription or Asset before the Order can complete. Not present on Configuration Orders. | Lines are created within the scope of the Order. See Commerce: Order Line canon — pending canonisation. |

| Commerce: Order | Child | One Order to many Attachments | Files or License Key text attached to the Order by Vendor or Operations. Attachments can be added in any status except Failed and Deleted. | See Commerce: Order Attachment canon — pending canonisation. |
...
| Accounts: Account | Association | Many Orders to one Client Account | The Client Account on whose behalf the Order is placed. Derived from the Actor context at creation. | Immutable after Order creation. |

| Catalog: licensee (pending canonisation) | Association | Many Orders to one Licensee | The Licensee on whose behalf the Order is placed. The Seller of the Licensee must match the Seller of the Listing at Order creation. | Immutable after Order creation. |

| Catalog: buyer (pending canonisation) | Association | Many Orders to one Buyer | The Buyer associated with the Licensee. Derived from the Licensee at creation. | Immutable after Order creation. |
...
| Accounts: Account | Association | Many Orders to one Vendor Account | The Vendor Account for the Product being ordered. Derived from the Listing at creation. | Immutable after Order creation. |
...
| Purchase Order created (Draft or direct to Processing) | Commerce | Agreement: Agreement co-created in Draft status | No | Order type is Purchase | Agreement and Order are created simultaneously. |

| Any Order placed ( | Processing) → Commerce | Agreement: Agreement → Provisioning | No | Order type is Purchase | — |

| Any Order placed ( | Processing) → Commerce | Agreement: Agreement → Updating | No | Order type is Change, Configuration, or Termination | — |

| Change Order placed ( | Processing) → Commerce | Subscription: Affected pre-existing Subscriptions → Updating | No | Order type is Change; Subscriptions must be pre-existing | Newly created Draft Subscriptions are unaffected — they do not exist yet at placement. |

| Purchase or Change Order Processing | Commerce | Subscription: Draft Subscriptions created under the Order | No | Vendor Extension creates Draft Subscriptions during Processing | One or more Draft Subscriptions may be created per Order. |

| Purchase or Change Order Processing | Commerce | Asset: Draft Assets created under the Order | No | Vendor Extension creates Draft Assets during Processing where one-time purchase Items are present | One or more Draft Assets may be created per Order. |

| Purchase or Change Order completed ( | Completed) → Commerce | Agreement: Agreement → Active | No | Order type is Purchase or Change | — |

| Purchase or Change Order completed ( | Completed) → Commerce | Subscription: Draft Subscriptions → Active, linked to Agreement | No | All Lines must be mapped to a Subscription or Asset | — |

| Purchase or Change Order completed ( | Completed) → Commerce | Asset: Draft Assets → Active, linked to Agreement | No | All Lines must be mapped to a Subscription or Asset | — |

| Termination Order completed ( | Completed) → Commerce | Subscription: Terminated Subscriptions → Terminated | No | Order type is Termination | — |

| Termination Order completed ( | Completed) → Commerce | Agreement: Agreement → Terminated | No | All Subscriptions on the Agreement are Terminated | Only triggered if all Subscriptions are Terminated as a result of this Order. |

| Purchase Order completed ( | Completed) → Commerce | Agreement `parameters`: Agreement-scoped parameters from Purchase Order carried over to Agreement | No | Order type is Purchase; Order transitions to Completed | Both `parameters.ordering` and `parameters.fulfillment` Agreement-scoped values are persisted on the Agreement. Order-scoped parameters are not carried over. |

| Purchase Order failed ( | Failed) → Commerce | Agreement: Agreement → Failed | No | Order type is Purchase | — |

| Change, Configuration, or Termination Order failed ( | Failed) → Commerce | Agreement: Agreement → Active | No | Order type is Change, Configuration, or Termination | Agreement reverts to Active. All Subscriptions and Assets remain unchanged. |

| Draft or Quoted Purchase Order deleted ( | Deleted) → Commerce | Agreement: Agreement → Deleted | No | Order type is Purchase; Order must be in Draft or Quoted | Soft-deleted — Agreement remains retrievable via the API. |

| Any Order placed against an Agreement with competing Draft/Quoted Orders | Commerce | Order: Competing Draft and Quoted Orders → Deleted | No | Any Order placed against an Agreement that already has Draft or Quoted Orders | All competing Draft and Quoted Orders are soft-deleted simultaneously. |
...
Reversible: Processing → Querying → Processing is reversible with no limit on cycles. The Vendor may move an Order between Processing and Querying as many times as needed until the Order is Completed or Failed.

All other transitions are irreversible. Draft → Quoted is a one-way transition. Completed, Failed, and Deleted are terminal states with no outbound transitions. Deletion: Orders use a soft-delete model. Deleting an Order moves it to Deleted status — it is not permanently removed and remains retrievable via the API including in standard list responses. Only Orders in Draft or Quoted status can be deleted. This behaviour deviates from Platform Invariant 7 — see `PLATFORM_CANON_PREAMBLE.md`.

For Purchase Orders, deletion of a Draft or Quoted Order also moves the corresponding Draft Agreement to Deleted status under the same soft-delete model. Audit: The Order audit block captures timestamps and Actor references for key lifecycle events. In addition to the standard `created` and `updated` entries, the audit block includes state-specific entries that are populated as the Order progresses: `processing`, `querying`, `completed`, and `failed`. State-specific audit entries are only present if the Order has reached that state. The audit block is omitted from API responses by default — request via `select=+audit`.
...
| Client attempts to create Termination Order against Agreement with no non-Terminated Subscriptions | The platform rejects creation of the Termination Order. There is no active commercial relationship to terminate — all Subscriptions on the Agreement are already in Terminated state. The Client receives a validation error and the Order is not persisted. | Client | Low | Platform-enforced — see BR-003a. Typical cause: Client attempts to terminate an Agreement whose Subscriptions have already been terminated by a previous Termination Order. The Agreement itself may already be in Terminated state in this case. |
...
| --- | --- | --- | --- |
| 0.1 | auto | render.js | Auto-generated from graph. |
